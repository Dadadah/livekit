...
func Test_OnTrackSubscribed_EventIsSent(t *testing.T) {
	...
	// do
	fixture.sut.TrackSubscribed(context.Background(), livekit.RoomID(room.Sid), livekit.RoomName(room.Name), room.Metadata, livekit.ParticipantID(partSID), trackInfo, publisherInfo, true)
	time.Sleep(time.Millisecond * 500)
